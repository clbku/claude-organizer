...
- **Comments** — the implementing session tends to **over-comment**, so hunt this actively. Flag **every** comment that restates what the code does, narrates the obvious, banners a section (`// state`, `// helpers`), or notes "added for X"/a ticket ref — those are noise to delete. The bar: a comment earns its place only if removing it would leave a future session worse off (a non-obvious *why*, a subtle invariant, a bug/quirk workaround, a public-API doc); never flag one that clears it. The **same rationale copy-pasted across files** is one finding, not many — keep it once, drop the rest.
